...
const FacialRegistration = () => {
    ...
    const captureAndDetectFace = async () => {
        if (webcamRef.current && canvasRef.current) {
            ...
            try {
                ...
                const response = await fetch('https://tda-vector-127354774628.asia-southeast1.run.app/detect_faces_fxn', {
                    method: 'POST',
    ...
    const captureAndSaveImage = async () => {
        ...
        try {
            const response = await fetch('https://tda-vector-127354774628.asia-southeast1.run.app/save_embedding', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
